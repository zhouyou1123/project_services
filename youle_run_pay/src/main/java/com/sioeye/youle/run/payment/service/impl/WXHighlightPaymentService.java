...
@Service
@RefreshScope
public class WXHighlightPaymentService implements IWXHighlightPayment {
	...
	public JSONObject prePayment(Map<String, Object> map) throws CustomException, Exception {
		...
		if ("SUCCESS".equals(returnMap.get("return_code")) && "SUCCESS".equals(returnMap.get("result_code"))) {
			...
			this.insertPayment(sioeyeSign, map.get("openId").toString(), returnMap, result, payment);
			return result;
		}
		throw new CustomException(EnumHandle.WEIXIN_RETURN_PARAMS_ERROR);
	}
	...
	private boolean validatePrice(String sioeyeSign, String nonceStr, Map<String, Object> map, JSONObject result)
		...
		if (Double.parseDouble(map.get("actualAmount").toString()) == 0) {
			...
			// 调用插入payment方法
			insertPayment(sioeyeSign, map.get("openId").toString(), returnMap, result, payment);
			return true;
		}
		return false;
	}
	 ...
	 * @throws CustomException
	 */
	private void insertPayment(String sioeyeSign, String openId, SortedMap<String, String> map, JSONObject result,
		...
		secondParameters.put("package=prepay_id", map.get("prepay_id"));
		...
		payment.setTradeType(EnumTradeType.HIGHLIGHT_APPLET_DISNEY.toString());
		payment.setPrepayId(map.get("prepay_id"));

...
@Service
@RefreshScope
public class WXH5PaymentService implements IWXH5Payment {
	...
	@Value("${weixin.h5.redirect.url}")
	private String weixinH5RedirectUrl;
	@Autowired
	...
	public JSONObject prePayment(Map<String, Object> map) throws CustomException, Exception {
		...
		parameters.put("scene_info",
				"{\"h5_info\": {\"type\":\"Wap\",\"wap_url\": \"https://line-h5.sioeye.com\",\"wap_name\": \"喜爱搜脸视频下载\"}}");
		...
		if ("SUCCESS".equals(returnMap.get("return_code")) && "SUCCESS".equals(returnMap.get("result_code"))) {
			...
			this.insertPayment(sioeyeSign, returnMap, result, payment);
			return result;
		}
		throw new CustomException(EnumHandle.WEIXIN_RETURN_PARAMS_ERROR);
	}
	 ...
	 * @throws CustomException
	 */
	private void insertPayment(String sioeyeSign, SortedMap<String, String> map, JSONObject result, Payment payment)
			throws CustomException, Exception {
		SortedMap<Object, Object> secondParameters = new TreeMap<Object, Object>();
		secondParameters.put("appid", sioeyeWeixinAppletId);
		secondParameters.put("partnerid", sioeyeWeixinAppletMchId);
		secondParameters.put("prepayid", map.get("prepay_id"));
		secondParameters.put("noncestr", map.get("nonce_str"));
		secondParameters.put("timestamp", System.currentTimeMillis() / 1000);
		secondParameters.put("package", "Sign=WXPay");
		...
		payment.setNonceStr(secondParameters.get("noncestr").toString());
		payment.setTradeType(EnumTradeType.H5_APPLET_DISNEY.toString());
		payment.setPrepayId(secondParameters.get("prepayid").toString());
		...
		// 封装返回值
//		result.put("mwebUrl", map.get("mweb_url") + URLEncoder.encode(weixinH5RedirectUrl, "GBK"));
		result.put("mwebUrl", map.get("mweb_url"));
	...
	private boolean sendMqMessage(Map<String, String> map) throws IOException, TimeoutException {
		...
		message.put("payWay", map.get("payWay"));
		message.put("tradeType", map.get("tradeType"));
